...
5. **Link the Jira issue.** Find the issue key (branch name, commit messages, or ask) and link it in the description; add any related PRs or docs.
6. **Create the PR as a draft.** Default to a **draft** PR unless told otherwise — `gh pr create --draft`, or the GitHub MCP tool's `draft: true`. Prefer the MCP tool; otherwise `gh pr create`. Target the correct base (`main` unless told otherwise). Report the PR URL.
