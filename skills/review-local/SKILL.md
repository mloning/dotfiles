...
2. **Launch both reviews in parallel.** Run two independent reviews concurrently against the checklist in steps 3–6, then combine at the end (step 7) — don't run them sequentially:
   ...
   git diff HEAD >> /tmp/review-diff.txt   # include uncommitted work too
...
7. **Combine results.** Once your own review is done, `wait` for the background reviewer and read `/tmp/review-other.json`:
