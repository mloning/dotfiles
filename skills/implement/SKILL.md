...
1. **Start from the approved plan.** Work from the plan at `plans/<feature-slug>.md` (or the direction agreed in context). Re-read it so the slices, success criteria, and non-goals are fresh. If there's no approved plan yet, stop and say so — brainstorm and plan first; don't improvise one here.
...
5. **Cross-agent review.** Once all slices are done, follow `review-local` (you plus the other agent in parallel). Fix all Critical and Important findings before submitting.
