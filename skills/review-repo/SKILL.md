...
3. **Launch both reviews in parallel.** Run two independent reviews against steps 4–8 and combine at the end (step 9):
   ...
   printf '%s' '{"type":"object","additionalProperties":false,"required":["findings","verdict"],"properties":{"findings":{"type":"array","items":{"type":"object","additionalProperties":false,"required":["severity","location","issue","why","fix"],"properties":{"severity":{"type":"string","enum":["high","medium","low"]},"location":{"type":"string"},"issue":{"type":"string"},"why":{"type":"string"},"fix":{"type":"string"}}}},"verdict":{"type":"string"}}}' > /tmp/review-schema.json
   ...
   SCOPE: $FOCUS" < /dev/null) > /tmp/review-other.log 2>&1 &
   ...
   SCOPE: $FOCUS" \
       ...
       --output-format json --no-session-persistence < /dev/null) > /tmp/review-other.json 2>&1 &
   ```

   The reviewer roams, so it can run longer than the diff-based skills — the 240s `perl alarm` is a backstop. Because roaming is open-ended, the kill can still lose output; the focused SCOPE is what keeps it finishing in time. Keep reviewing while it runs, then `wait`.
...
9. **Combine results.** Once your review is done, `wait` for the background reviewer and read `/tmp/review-other.json`:
